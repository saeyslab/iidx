...
message('TNK DA regular')
Sys.setenv(IIDX_EXTRA_COVARIATE = ''); res_da <- test_da(counts = inputs$counts, annotation = annotation, predictors = predictors, confounders = confounders, interactions = FALSE, batch_aware = TRUE, verbose = FALSE, parallel = TRUE)
saveRDS(res_da, '../NEW RESULTS/TNK_DA_Full_Regular.RDS'); rm(res_da); gc(); gc(); gc()
message('TNK DA interactions')
Sys.setenv(IIDX_EXTRA_COVARIATE = ''); res_da <- test_da(counts = inputs$counts, annotation = annotation, predictors = predictors, confounders = confounders, interactions = TRUE, batch_aware = FALSE, verbose = FALSE, parallel = TRUE)
saveRDS(res_da, '../NEW RESULTS/TNK_DA_Full_Interactions.RDS'); rm(res_da); gc(); gc(); gc()

message('TNK DS-MFI regular')
Sys.setenv(IIDX_EXTRA_COVARIATE = ''); res_ds_mfi <- test_ds(annotation = annotation, mfi = inputs$MFIs, counts = inputs$counts, predictors  = predictors, confounders = confounders, interactions = FALSE, batch_aware = TRUE, state_markers = as.vector(channels)[idcs_channels_state], verbose = FALSE, parallel = TRUE)
saveRDS(res_ds_mfi, '../NEW RESULTS/TNK_DS-MFI_Regular.RDS'); rm(res_ds_mfi); gc(); gc(); gc()
message('TNK DS-MFI interactions')
Sys.setenv(IIDX_EXTRA_COVARIATE = ''); res_ds_mfi <- test_ds(annotation = annotation, mfi = inputs$MFIs, counts = inputs$counts, predictors = predictors, confounders = confounders, interactions = TRUE, batch_aware = FALSE, state_markers = as.vector(channels)[idcs_channels_state], verbose = FALSE, parallel = TRUE)
saveRDS(res_ds_mfi, '../NEW RESULTS/TNK_DS-MFI_Interactions.RDS'); rm(res_ds_mfi); gc(); gc(); gc()

message('TNK DS-Pheno regular')
Sys.setenv(IIDX_EXTRA_COVARIATE = ''); res_ds_pheno <- test_ds(annotation = annotation, phenopos = inputs$PercPos, counts = inputs$counts, predictors = predictors, confounders = confounders, interactions = FALSE, batch_aware = TRUE, state_markers = as.vector(channels)[idcs_channels_state], verbose = TRUE, parallel = TRUE)
saveRDS(res_ds_pheno, '../NEW RESULTS/TNK_DS-Pheno_Regular.RDS'); rm(res_ds_pheno); gc(); gc(); gc()
message('TNK DS-Pheno interactions')
Sys.setenv(IIDX_EXTRA_COVARIATE = ''); res_ds_pheno <- test_ds(annotation = annotation, phenopos = inputs$PercPos, counts = inputs$counts, predictors = predictors, confounders = confounders, interactions = TRUE, batch_aware = FALSE, state_markers = as.vector(channels)[idcs_channels_state], verbose = TRUE, parallel = TRUE)
saveRDS(res_ds_pheno, '../NEW RESULTS/TNK_DS-Pheno_Interactions.RDS'); rm(res_ds_pheno); gc(); gc(); gc()
...
saveRDS(res_da, '../NEW RESULTS/TNK_DA_Cohort_VRC.RDS'); rm(res_da); gc(); gc(); gc()
message('TNK DA cohort: UK')
...
saveRDS(res_da, '../NEW RESULTS/TNK_DA_Cohort_UK.RDS'); rm(res_da); gc(); gc(); gc()
message('TNK DA cohort: Twin1')
...
saveRDS(res_da, '../NEW RESULTS/TNK_DA_Cohort_Twin1.RDS'); rm(res_da); gc(); gc(); gc()
message('TNK DA cohort: Twin2')
...
saveRDS(res_da, '../NEW RESULTS/TNK_DA_Cohort_Twin2.RDS'); rm(res_da); gc(); gc(); gc()
message('TNK DA cohort: VRC+Twin1')
...
saveRDS(res_da, '../NEW RESULTS/TNK_DA_Cohort_VRC+Twin1.RDS'); rm(res_da); gc(); gc(); gc()
message('TNK DA cohort: VRC+Twin2')
...
saveRDS(res_da, '../NEW RESULTS/TNK_DA_Cohort_VRC+Twin2.RDS'); rm(res_da); gc(); gc(); gc()

message('TNK DS-MFI cohort: VRC')
...
saveRDS(res_ds_mfi, '../NEW RESULTS/TNK_DS-MFI_Cohort_VRC.RDS'); rm(res_ds_mfi); gc(); gc(); gc()
message('TNK DS-MFI cohort: UK')
...
saveRDS(res_ds_mfi, '../NEW RESULTS/TNK_DS-MFI_Cohort_UK.RDS'); rm(res_ds_mfi); gc(); gc(); gc()
message('TNK DS-MFI cohort: Twin1')
...
saveRDS(res_ds_mfi, '../NEW RESULTS/TNK_DS-MFI_Cohort_Twin1.RDS'); rm(res_ds_mfi); gc(); gc(); gc()
message('TNK DS-MFI cohort: Twin2')
...
saveRDS(res_ds_mfi, '../NEW RESULTS/TNK_DS-MFI_Cohort_Twin2.RDS'); rm(res_ds_mfi); gc(); gc(); gc()
message('TNK DS-MFI cohort: VRC+Twin1')
...
saveRDS(res_ds_mfi, '../NEW RESULTS/TNK_DS-MFI_Cohort_VRC+Twin1.RDS'); rm(res_ds_mfi); gc(); gc(); gc()
message('TNK DS-MFI cohort: VRC+Twin2')
...
saveRDS(res_ds_mfi, '../NEW RESULTS/TNK_DS-MFI_Cohort_VRC+Twin2.RDS'); rm(res_ds_mfi); gc(); gc(); gc()

# message('TNK DS-Pheno cohort: VRC')
...
# saveRDS(res_ds_pheno, '../NEW RESULTS/TNK_DS-Pheno_Cohort_VRC.RDS'); rm(res_ds_pheno); gc(); gc(); gc()
# message('TNK DS-Pheno cohort: UK')
...
# saveRDS(res_ds_pheno, '../NEW RESULTS/TNK_DS-Pheno_Cohort_UK.RDS'); rm(res_ds_pheno); gc(); gc(); gc()
# message('TNK DS-Pheno cohort: Twin1')
...
# saveRDS(res_ds_pheno, '../NEW RESULTS/TNK_DS-Pheno_Cohort_Twin1.RDS'); rm(res_ds_pheno); gc(); gc(); gc()
# message('TNK DS-Pheno cohort: Twin2')
...
# saveRDS(res_ds_pheno, '../NEW RESULTS/TNK_DS-Pheno_Cohort_Twin2.RDS'); rm(res_ds_pheno); gc(); gc(); gc()
# message('TNK DS-Pheno cohort: VRC+Twin1')
...
# saveRDS(res_ds_pheno, '../NEW RESULTS/TNK_DS-Pheno_Cohort_VRC+Twin1.RDS'); rm(res_ds_pheno); gc(); gc(); gc()
# message('TNK DS-Pheno cohort: VRC+Twin2')
...
# saveRDS(res_ds_pheno, '../NEW RESULTS/TNK_DS-Pheno_Cohort_VRC+Twin2.RDS'); rm(res_ds_pheno); gc(); gc(); gc()
...
saveRDS(res_da, '../NEW RESULTS/TNK_DA_CMV_Age.RDS'); rm(res_da); gc(); gc(); gc()
...
saveRDS(res_da, '../NEW RESULTS/TNK_DA_CMV_Sex.RDS'); rm(res_da); gc(); gc(); gc()
message('TNK DS-MFI CMV')
...
saveRDS(res_ds_mfi, '../NEW RESULTS/TNK_DS-MFI_CMV_Age.RDS'); rm(res_ds_mfi); gc(); gc(); gc()
...
saveRDS(res_ds_mfi, '../NEW RESULTS/TNK_DS-MFI_CMV_Sex.RDS'); rm(res_ds_mfi); gc(); gc(); gc()
# message('TNK DS-Pheno CMV')
...
# saveRDS(res_ds_pheno, '../NEW RESULTS/TNK_DS-Pheno_CMV.RDS'); rm(res_ds_pheno); gc(); gc(); gc()
